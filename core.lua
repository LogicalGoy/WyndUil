...
function Module.Init(cfg)
	...
	-- Re-running a script without unloading it used to stack a whole second
	-- copy on top of the first: new window, new island, new loops, and nothing
	-- pointing at the old ones. Two hundred executes later the client is
	-- rendering two hundred menus and looks like it is doing nothing at all.
	-- Anything that got this far replaces whatever came before it.
	do
		local prev = rawget(_G, "__SupraCore")
		if prev and prev.Cleanup and not prev.Cleanup.Dead then
			pcall(function() prev.Cleanup:Destroy() end)
		end
		_G.__SupraCore = Core
	end
	...
	do
		...
		function Island:AttachWindow(window, expectedSize)
			...
			-- told, otherwise it sits on top of the menu until you click it.
			Island:SetMenuOpen(true)
